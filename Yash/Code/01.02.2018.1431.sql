...
SELECT * FROM Customers
WHERE Country IN (SELECT Country FROM Suppliers);/*This will show all the countries in another database. In this case we are calling Suppliers database's countries from the Customers database. This is where this statement comes in handy.*/
